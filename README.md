...
Website profil & promosi untuk **PT Sanjaya Mirai Hidroponik** (Kediri) — menampilkan
produk melon & sayuran hidroponik, wisata petik melon, dan memudahkan pemesanan via WhatsApp.

Dibuat dengan **Astro** (ringan & cepat) + **Decap CMS** (agar pemilik bisa mengedit
...
1. Daftar di **https://github.com** → gunakan email Mas Shandi.
2. Daftar di **https://netlify.com** → pilih **"Sign up with GitHub"** (agar terhubung).

### Langkah 2 — Unggah kode ke GitHub
1. Di GitHub, klik **New repository** → beri nama misalnya `sanjaya-hidroponik` → **Create**.
...
1. Login Netlify → **Add new site** → **Import an existing project** → **GitHub**.
2. Pilih repository `sanjaya-hidroponik`.
3. Netlify otomatis membaca file `netlify.toml`. Pastikan:
   - **Build command:** `npm run build`
   - **Publish directory:** `dist`
4. Klik **Deploy**. Tunggu 1–2 menit — website langsung tayang di alamat seperti
...
1. Di dashboard Netlify site Anda → menu **Integrations / Identity** →
   klik **Enable Identity**.
2. Masih di Identity → **Services → Git Gateway → Enable Git Gateway**.
3. Di bagian **Identity → Registration**, pilih **Invite only** (agar hanya orang yang
   diundang yang bisa masuk — lebih aman).
4. Klik **Invite users** → masukkan **email Mas Shandi** → kirim.
5. Mas Shandi cek email → klik **Accept the invite** → buat password.
6. Selesai! Login kapan saja di **`https://ALAMAT-ANDA.netlify.app/admin`**.

---

## ✏️ Cara Mengedit Konten (untuk Mas Shandi — mudah)

1. Buka `https://ALAMAT-ANDA.netlify.app/admin` → **Login** (email & password).
2. Ada 2 menu:
   ...
   - **Pengaturan Situs** → ubah teks utama, **nomor WhatsApp**, alamat, jam buka,
     link Instagram, teks tentang, keunggulan, dll.
3. Untuk **ganti foto**: klik kolom gambar → **Upload** foto baru dari HP/komputer.
4. Setelah selesai → klik **Publish** (atau **Save** lalu **Publish** bila memakai mode draf).
5. Website akan **otomatis diperbarui** dalam 1–2 menit. Tidak perlu menyentuh kode.
...
Sebaiknya nanti diganti/dilengkapi:
...
## ℹ️ Data Perusahaan (tersimpan di `src/data/site.json`)
